...
public class PostsController {
    ...
    public static final String IMAGE_FILE_INVALID_MESSAGE = "You need to add photos";
    ...
    public ModelAndView showPosts(ModelAndView modelAndView, @PageableDefault(value = PAGE_NUMBER_TO_PRESENT) Pageable pageable) {
        ...
        modelAndView.setViewName(PostViewUri.POSTS_INDEX);
        return modelAndView;
    }
    ...
    public ModelAndView showCreateForm(ModelAndView modelAndView, Model model) {
        ...
        modelAndView.setViewName(PostViewUri.POSTS_CREATE);
        return modelAndView;
    }
    ...
    @PostMapping("/{id}/edit")
    public String editPost(@Valid @ModelAttribute PostDTO postDTO, BindingResult bindingResult, @PathVariable Long id){
        iPostService.checkUploadImageInvalid(postDTO, bindingResult);
        if (isBindingError(bindingResult)) return PostViewUri.POSTS_EDIT;
